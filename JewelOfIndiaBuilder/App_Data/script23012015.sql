...
USE [JewelofIndia]
GO
/****** Object:  StoredProcedure [dbo].[sp_InsertApartmentSalesStatus]    Script Date: 01/27/2015 22:42:59 ******/
SET ANSI_NULLS ON
GO
...
-- Description:	<Description,,>
-- =============================================
ALTER PROCEDURE [dbo].[sp_InsertApartmentSalesStatus]
	-- Add the parameters for the stored procedure here
	@ApartmentId BIGINT,	
	@UserId BIGINT
...
AS
	...
	SET NOCOUNT ON;
	DECLARE @isAdmin BIT
	DECLARE @salesType SMALLINT
	SET @isAdmin=0
	SELECT @isAdmin=IsOwner FROM dbo.Users WITH(NOLOCK) WHERE id = @UserId
	IF @isAdmin =0
	SELECT @salesType = id FROM dbo.ApartmentSalesType WHERE SalesType = 'Request For Hold'
	ELSE
	SELECT @salesType = id FROM dbo.ApartmentSalesType WHERE SalesType = 'Hold'
	
    -- Insert statements for procedure here
	INSERT INTO dbo.ApartmetSales
        ( ApartmentId ,
		  IsBlocked,
		  UserId,
		  BlockStartDate,
...
VALUES  ( @ApartmentId , -- ApartmentId - bigint
          @isAdmin , -- IsBlocked - bit
          @UserId, -- UserId - bigint
          GETDATE() , -- BlockStartDate - datetime
          DATEADD(DAY,10,GETDATE()), -- BlockEndDate - datetime
          @salesType
        )
